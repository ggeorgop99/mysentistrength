...
def plot_predicted_probabilities(Y_probabilities):
    plt.figure(figsize=(10, 6))
    plt.hist(Y_probabilities, bins=50, alpha=0.75, color='blue', label='Predicted probabilities')
...
def plot_roc_curve(Y_test, Y_predictions, roc_auc, mode):
    fpr, tpr, thresholds = roc_curve(Y_test, Y_predictions if mode == "bin" else Y_predictions[:, 1])
...
def plot_roc_curve_float(Y_test, Y_probabilities, roc_auc_float, mode):
    fpr, tpr, thresholds = roc_curve(Y_test, Y_probabilities if mode == "bin" else Y_probabilities[:, 1])
    # Determine the optimal threshold (e.g., the one that maximizes the TPR - FPR difference)
    optimal_idx = np.argmax(tpr - fpr)
...
def calculate_metrics(Y_test, Y_predictions, Y_probabilities, mode):
    ...
    weighted_accuracy = np.sum(correct_predictions * weights.flatten()) / np.sum(weights.flatten())
    ...
    precision = precision_score(Y_test, Y_predictions, average='binary' if mode == "bin" else 'macro')
    recall = recall_score(Y_test, Y_predictions, average='binary' if mode == "bin" else 'macro')
...
roc_auc, roc_auc_float, recall = calculate_metrics(Y_test, Y_predictions, Y_probabilities, mode)
